...
namespace HTC.UnityPlugin.Multimedia
{
	public class ViveMediaDecoder : MonoBehaviour
	{
		...
		private const int AUDIO_FRAME_SIZE = 4096;  //  Audio clip data size. Packed from audioDataBuff.
        ...
        private const double OVERLAP_TIME = 0.2;   //  Our audio clip is defined as: [overlay][audio data][overlap].
		...
		public float frameID = 0;

        void Awake () {
			yuv = new Material(Resources.Load<Shader>("YUVConvert"));
			output = new RenderTexture(3840, 2160, 0, RenderTextureFormat.ARGBFloat);
		...
		{
			if (videoTexYch == null) return;
			if (videoTexYch.width != output.width && videoTexYch.height != output.height)
			{
				output = new RenderTexture(videoTexYch.width, videoTexYch.height, 0, RenderTextureFormat.ARGBFloat);
				Debug.Log("New output rendertexture created: " + videoTexYch.width + " - " + videoTexYch.height);
			}
			Graphics.Blit(null, output, yuv);
		}
        ...
        private void initAudioSource() {
            ...
            for (int i = 0; i < SWAP_BUFFER_NUM; i++) {
                ...
                audioSource[i].clip = AudioClip.Create("AUDIO" + i, audioDataLength, audioChannels, audioFrequency, false);
		...
		private void getTextureFromNative() {
			...
			videoTexYch = Texture2D.CreateExternalTexture(
				videoWidth, videoHeight, TextureFormat.RGBAFloat, false, false, nativeTexturePtrY);
			videoTexUch = Texture2D.CreateExternalTexture(
				videoWidth / 2, videoHeight / 2, TextureFormat.RGBAFloat, false, false, nativeTexturePtrU);
			videoTexVch = Texture2D.CreateExternalTexture(
				videoWidth / 2, videoHeight / 2, TextureFormat.RGBAFloat, false, false, nativeTexturePtrV);
		}
		...
		private void setTextures(Texture ytex, Texture utex, Texture vtex) {
			yuv.SetTexture("_YTex", ytex);
			yuv.SetTexture("_UTex", utex);
			yuv.SetTexture("_VTex", vtex);
		}
